...
const mockSponsors: Sponsor[] = [
  { id: 1, name: 'Breakwater Hotel', logo_url: '/images/breakwater.avif', website_url: 'https://example.com', tier: 'gold', display_order: 1, is_active: true, created_at: new Date().toISOString() },
  { id: 2, name: 'Meys Meats', logo_url: '/images/Meys-Logo-web_png.avif', website_url: 'https://example.com', tier: 'gold', display_order: 2, is_active: true, created_at: new Date().toISOString() },
  { id: 3, name: 'Bunnings', logo_url: '/images/Bunnings-logo_edited.avif', website_url: 'https://example.com', tier: 'silver', display_order: 1, is_active: true, created_at: new Date().toISOString() },
  { id: 4, name: 'Rotary Club', logo_url: '/images/rotary club_edited.avif', website_url: 'https://example.com', tier: 'silver', display_order: 2, is_active: true, created_at: new Date().toISOString() },
  { id: 5, name: 'Repco', logo_url: '/images/repco.avif', website_url: 'https://example.com', tier: 'bronze', display_order: 1, is_active: true, created_at: new Date().toISOString() },
  { id: 6, name: 'Lektrix', logo_url: '/images/Lektrix_edited_edited.avif', website_url: 'https://example.com', tier: 'bronze', display_order: 2, is_active: true, created_at: new Date().toISOString() },
];
...
export const sponsorKeys = {
  ...
  byTier: (tier: SponsorTier) => [...sponsorKeys.all, 'tier', tier] as const,
};
...
export function useSponsorsByTier(tier: SponsorTier) {
  return useQuery({
    queryKey: sponsorKeys.byTier(tier),
    queryFn: async () => {
      try {
        const response = await sponsorsApi.getByTier(tier);
        if (!response.success) throw new Error(response.error);
        return response.data!;
      } catch {
        if (DEV_MODE) return mockSponsors.filter(s => s.tier === tier);
        throw new Error('Failed to load sponsors');
      }
    },
  });
